# 問題：ユニークペアカウント（Two Sum Count）
# 問題文
# 整数配列 a と整数 k が与えられます。
# a[i] + a[j] = k となるユニークなペアの数を返してください。
# ペアは値の組 {x, y}（x ≤ y）として扱い、同じ組は1回だけカウントします。
# ⸻
# 制約
# 	•	1 ≤ a.length ≤ 10^5
# 	•	-10^9 ≤ a[i] ≤ 10^9
# 	•	-10^9 ≤ k ≤ 10^9
# ⸻
# ヒント（重要）
# 	•	インデックスではなく「値」でペアを数える
# 	•	重複する値がある場合の扱いに注意
# 	•	x == y の場合は特別な処理が必要
# ⸻
# 考えるべき問い（これが本質）
# 各値 x に対して、k - x が配列に存在するか？
# 重複カウントを避けるにはどうするか？
# ⸻
# 解けたら次に進むこと
# 	•	なぜ y < x をスキップするのか
# 	•	x == y の時に cnt >= 2 が必要な理由
# 	•	ハッシュマップの計算量の利点
# ⸻
def count_unique_pairs(a, k)
  freq = Hash.new(0)
  a.each { |v| freq[v] += 1 }
  ans = 0
  freq.each do |x, cnt|
    y = k - x
    next if y < x
    if x == y
      ans += 1 if cnt >= 2
    else
      ans += 1 if freq.key?(y)
    end
  end
  ans
end

p count_unique_pairs([1,1,2,2], 3) # 1
p count_unique_pairs([1,2,3,4], 5) # 2
p count_unique_pairs([0,0,0], 0)   # 1
p count_unique_pairs([1,1,1], 2)   # 1
